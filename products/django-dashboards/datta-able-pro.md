...
This feature allows to expose a model via DRF in three simple steps:

* Add model definition in `apps/models.py` (`Books` sample provided)
* Enable the DRF exposure in config
  * `DYNAMIC_API` variable
* Migrate the application
  * `$ python manage.py makemigrations`
  * `$ python manage.py migrate`

At this point, the new model is automaticaly exposed to address: `http://127.0.0.1:8000/api/<MODEL_NAME>/`.

Here is the [LIVE DEMO](https://django-datta-able-pro.appseed-srv1.com/api/books/) for a simple `Books` model with definition:
...
And the DRF interface:

![devtools-dyn-api-drf-ui](https://user-images.githubusercontent.com/51070104/171259417-eb886516-ff12-43a6-aa86-a7eaee46e56d.jpg)
